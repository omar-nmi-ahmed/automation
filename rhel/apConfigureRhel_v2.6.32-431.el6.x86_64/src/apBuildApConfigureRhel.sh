...
function ExecuteInstallationConfigurationSteps()
{
    ...
    cp -f public-yum-ol6.repo $AP_BIN
    chown aproject:aproject $AP_BIN/public-yum-ol6.repo
    chmod 444 $AP_BIN/public-yum-ol6.repo
   
    ApOutput "... Copying RPM-GPG-KEY-oracle to"
    ApOutput "... $AP_BIN."
    cp -f RPM-GPG-KEY-oracle $AP_BIN
    chown aproject:aproject $AP_BIN/RPM-GPG-KEY-oracle
    chmod 444 $AP_BIN/RPM-GPG-KEY-oracle

    ApOutput "... Copying RPM-GPG-KEY-oracle-ol6 to"
    ApOutput "... $AP_BIN."
    cp -f RPM-GPG-KEY-oracle-ol6 $AP_BIN
    chown aproject:aproject $AP_BIN/RPM-GPG-KEY-oracle-ol6
    chmod 444 $AP_BIN/RPM-GPG-KEY-oracle-ol6

    ApOutput "... Copy RhelRPmList to"
    ApOutput "... $AP_BIN."
    ApOutput "... RhelRpmList.txt contains the list of RPMs to be installed on RHEL."
    cp -f rhelRpmList.txt $AP_BIN
    chown aproject:aproject $AP_BIN/rhelRpmList.txt
    chmod 444 $AP_BIN/rhelRpmList.txt
    ...
    ApOutput "... $AP_BIN."
    cp -f apConfigureRhel.sh $AP_BIN
    chown aproject:aproject $AP_BIN/apConfigureRhel.sh
    chmod 555 $AP_BIN/apConfigureRhel.sh
